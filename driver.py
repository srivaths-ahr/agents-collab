...
Sequences stateless one-shot agents around a shared git workspace:
    ...
    EXECUTE  (executor) reads plan.md (+ AGENTS.md) -> edits files
                        (pluggable: cursor | claude | codex | gemini | antigravity)
...
Each role's MODEL is a variable you fill before running (or override on the CLI).
...
Nothing here is destructive: it stages changes (git add -A) to compute diffs but
...
PLAN_CLAUDE_MODEL_NAME         = "opus"      # planning: strong, long-thinking (always Claude)
VERIFICATION_CLAUDE_MODEL_NAME = "haiku"     # verification: cheap, bounded   (always Claude)
...
EXECUTOR_MAX_RETRIES   = 1    # extra attempts on a TRANSIENT executor failure (hang/timeout/nonzero)
...
INTERACTIVE_CLARIFY = True    # TTY: ask questions live; non-TTY: write questions to a file and halt

# ---- LOOP BUDGET / STOP GUARDS ----
MAX_ITERATIONS        = 8     # hard cap; loop stops even if not "pass"
MAX_IDENTICAL_FAILURES = 2    # stop if the loop stalls (same failure / no new diff) this many times

# ---- TEST GATE (the objective half of verification) ----
# Deterministic command the driver runs after each execute. Its pass/fail is
# ground truth handed to the verifier. Empty string = skip, judge on diff alone.
...
CLARIFY_NEEDED_FILE = "clarifications_needed.json"   # questions written here in unattended (non-TTY) mode
PROMPTS_DIR  = "prompts"
...
# ---- PERMISSIONS / TOOLS ----
# PLAN is read-only on purpose (it writes nothing; the driver writes plan.md from
...
# VERIFY only needs to read the scratch files; --bare keeps it lean and predictable.
...
def run(cmd, *, timeout, cwd=REPO_ROOT, stdin=None):
    ...
    try:
        proc = subprocess.run(
            cmd, cwd=cwd, input=stdin, timeout=timeout,
...
def run_claude(prompt, *, model, system_prompt_file, allowed_tools,
               bare=False, skip_permissions=False, timeout=CLAUDE_TIMEOUT):
    ...
    try:
        ...
        raise StepError(f"claude returned non-JSON envelope (rc={rc}). stderr:\n{err}\nstdout:\n{out[:800]}")
    return {
...
def run_executor(prompt, *, backend, model, timeout=CURSOR_TIMEOUT):
    """Invoke the chosen executor CLI headless, edits auto-applied. Blocks until
    exit. The contract is identical across backends: read plan.md, edit files in
    ...
    try:
        ...
        raise FatalError(f"unknown executor backend '{backend}'. "
...
def staged_diff_against(baseline):
    """Stage everything and return the cumulative diff vs the loop's start commit.
    Staging is how we capture new/untracked files in the diff; we never commit."""
    git("add", "-A")
...
def plan_step(iteration, prev_verdict):
    ...
    if os.path.exists(CLARIFY_FILE):
        payload.append(f"- {CLARIFY_FILE} (human answers to clarifying questions — authoritative)")
...
def execute_step(iteration):
    banner(f"ITERATION {iteration} — EXECUTE  "
    ...
    for attempt in range(1, attempts + 1):
        try:
            rc, output, cost = run_executor(
                prompt, backend=EXECUTOR_BACKEND, model=IMPLEMENTATION_MODEL_NAME)
        ...
        if attempt < attempts:
            log(f"execute attempt {attempt}/{attempts} failed: {last_err}; "
                f"retrying in {EXECUTOR_RETRY_BACKOFF}s", prefix="↻")
            time.sleep(EXECUTOR_RETRY_BACKOFF)
    raise StepError(f"executor failed after {attempts} attempt(s): {last_err}. "
...
def verify_step(iteration, baseline):
    ...
    log(f"diff={len(diff)} chars | tests: {'PASSED' if passed else 'FAILED' if ran else 'SKIPPED'}")
    ...
    if not diff.strip():
        ...
        return {"status": "blocked",
                ...
                "criteria": [], "tests": {"ran": ran, "passed": passed, "summary": "no diff"},
                "_no_diff": True}, 0.0
    ...
    try:
        ...
        raise StepError(f"verifier did not return valid JSON. Raw saved to {WORK_DIR}/verify_raw.txt")

    if "status" not in verdict:
        raise StepError(f"verdict JSON missing 'status'. Raw saved to {WORK_DIR}/verify_raw.txt")
...
def clarify_gate():
    ...
    for round_no in range(1, CLARIFY_MAX_ROUNDS + 1):
        ...
        existing = read_file(CLARIFY_FILE) if os.path.exists(CLARIFY_FILE) else "# Clarifications\n"
...
def halt_needs_clarification(nc, total_cost):
    ...
    if nc.questions:
        log("answer these (edit task.md, or add answers to clarifications.md), then re-run:", prefix="?")
    ...
    write_file(CLARIFY_NEEDED_FILE, json.dumps(
        {"issues": nc.issues, "questions": nc.questions,
         "assumptions_if_unanswered": nc.assumptions}, indent=2) + "\n")
...
def preflight():
    ...
    if not os.path.exists(os.path.join(REPO_ROOT, TASK_FILE)):
        ...
        raise FatalError(f"{TASK_FILE} did not exist — I wrote a template there. "
                         f"Fill in the goal and acceptance criteria, then re-run.")
    missing = [p for p in (CONTEXT_FILE,
...
def parse_cli_overrides():
    ...
    p.add_argument("--executor",     default=EXECUTOR_BACKEND,
                   choices=list(executors.EXECUTORS),
                   help="which coding CLI runs the execute step")
    p.add_argument("--impl-model",   default=IMPLEMENTATION_MODEL_NAME,
